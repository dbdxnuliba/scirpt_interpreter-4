...
unsigned int RobotState::packRobotMessage(uint8_t* buf, unsigned int offset)
{
    ...
    offset += RobotState::packRobotMessageVersion(buf, offset);
...
unsigned int RobotState::packRobotMessageVersion(uint8_t* buf, unsigned int offset)
{
    ...
    //TODO: the size of version_msg_.build_date
    memcpy(&buf[offset], &version_msg_.build_date, sizeof(version_msg_.build_date) - 1); //the last char '\0' is omitted
    offset += sizeof(version_msg_.build_date) - 1;
    return (offset - offset_);
}
